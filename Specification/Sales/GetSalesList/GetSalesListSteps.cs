...
[ Binding ]
public sealed class GetSalesListSteps
{
    ...
    {
        List<GetSalesListModel> models = table.CreateSet<GetSalesListModel>().ToList();

        for (int i = 0; i < models.Count; i++)
        {
            GetSalesListModel model = models[i];

            SalesListItemModel result = _results[i];
